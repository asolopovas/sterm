...
This skill is installed as a plain Agent Skills directory and is compatible with pi (`.agents/skills/tauri-v2`) and opencode (`.opencode/skills/tauri-v2`). When following links in this skill, resolve relative paths from the directory containing this `SKILL.md`.

For this repository, prefer the project workflow in `AGENTS.md`: use Bun/`just`, keep Tauri logic in Rust, mirror IPC types in `src/types.ts`, add command wrappers in `src/api.ts`, and update capabilities with least privilege. During a live Android dev session, do not run release or APK-replacing builds.
...
- Register every command in this project's `tauri::generate_handler![...]` / `invoke_handler![...]`
- Add the matching frontend wrapper in `src/api.ts` and mirrored types in `src/types.ts`
...
Check upstream docs when version-specific behaviour matters; this project currently carries Tauri 2.11 workarounds documented in `AGENTS.md`.
